...
type
  TTemplateSearchView = class(TForm)
    Panel1: TPanel;
...
var
  TemplateSearchView: TTemplateSearchView;

implementation

{$R *.dfm}

procedure TTemplateSearchView.btnImprimirClick(Sender: TObject);
var
  Caminho : String;
begin
...
procedure TTemplateSearchView.btnPesquisarClick(Sender: TObject);
begin
...
procedure TTemplateSearchView.btnTransferirClick(Sender: TObject);
begin
  if qryPesquisaPadrao.RecordCount > 0 then ModalResult := mrOK
  else Abort;
end;

procedure TTemplateSearchView.cbChavePesquisaChange(Sender: TObject);
begin
  edtNome.Text := '';
  edtInicio.Text := '';
  edtFim.Text := '';
...
procedure TTemplateSearchView.DBGrid1DblClick(Sender: TObject);
begin
  btnTransferir.Click;
end;

procedure TTemplateSearchView.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  qryPesquisaPadrao.Close;
end;

procedure TTemplateSearchView.FormCreate(Sender: TObject);
begin
...
procedure TTemplateSearchView.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
