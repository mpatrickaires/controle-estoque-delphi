...
uses
	...
	frxClass, frxDBSet, uView.Search.Template;

type
	TProductSearchView = class(TTemplateSearchView)
...
var
	ProductSearchView: TProductSearchView;

implementation

{$R *.dfm}

procedure TProductSearchView.btnImprimirClick(Sender: TObject);
begin
	NomeRelatorio := 'relProduto.fr3';
	inherited;
end;

procedure TProductSearchView.btnPesquisarClick(Sender: TObject);
begin
	inherited;
...
procedure TProductSearchView.btnTransferirClick(Sender: TObject);
begin
	inherited;
	Codigo := qryPesquisaPadraoID_PRODUTO.AsInteger;
end;

procedure TProductSearchView.cbChavePesquisaChange(Sender: TObject);
begin
	case cbChavePesquisa.ItemIndex of
		5 :
		begin
